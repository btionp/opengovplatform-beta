...
DataViz.queryBuilder = {
	...
	where_statement: function(space){
		...
		for(i in where_var) {
			if(typeof where_var[i].field != 'undefined' && typeof where_var[i].operator != 'undefined' && typeof where_var[i].value != 'undefined') {
				if(typeof where_var[i].value == 'string') {
					statement = where_var[i].field + ' ' + where_var[i].operator + ' "' + where_var[i].value + '"';
				} else {
					statement = where_var[i].field + ' ' + where_var[i].operator + ' ' + where_var[i].value;
				}
			} else {
				statement = where_var[i].statement;
			}
			
			if(i > 0) 
				query += ' '+ where_var[i].and_or + ' '+ statement  ;
			else  
				query += statement  ;
		}
