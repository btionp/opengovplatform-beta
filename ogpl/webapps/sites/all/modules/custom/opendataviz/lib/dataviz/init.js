var DataViz = {
	...
	setBaseLabel: function(base_label) {
		$('#chart_base_axis_label').val(DataViz._cache_data.getColumnLabel(parseInt(base_label)));
	},
	...
	createInstanceList: function(node_id) {
		$.ajax({
	         url: "http://demodatacms.nic.in/visualizationinstances/"+node_id,
	         type: "GET",
	         cache: false,
	...
	setDatasource: function(source, instance) {
		var loadinghtml = '<center><img src="images/loading.gif" /></center>';
		switch(source) {
			...
			case'data_gov_in_source':
				DataViz.query_instance = 'http://demodata.nic.in/opendatavisualization/' + instance + '/json';
				//$('#ModalDataGovIn').modal('hide');
				$('#data_viz_grid').html(loadinghtml);
				DataViz.drawVisualization();
				break;
			default:
				query_instance = 'http://10.1.36.45/googleviz/csvtable.php?tq=';
				$('#data_viz_grid').html(loadinghtml);
				DataViz.drawVisualization();
				break;
	...
	_generate_map: function(data, map, auto) {
		var geo_index = DataViz._get_lat_lon_index(data, auto);
		   ...
		   google.maps.ControlPosition.BOTTOM_LEFT = "<a href='http://www.nic.in'>Powered By NIC Map Service</a>";
	...
	_initialize_map: function(data, cache, auto) {
        ...
        var map = new google.maps.Map(mapDiv, {
          ...
          mapTypeId: google.maps.MapTypeId.ROADMAP,//'NICMap'
       ...
       /* var url = 'http://10.1.31.19/ArcGIS/rest/services/nicgis/MapServer';
		...
		*/
        //var url = 'http://10.1.31.19/ArcGIS/rest/services/nicgis/MapServer';
        //var url = 'http://10.1.31.19/ArcGIS/rest/services/ind_datagov/MapServer';
        //var url = 'http://server.arcgisonline.com/ArcGIS/rest/services/World_Topo_Map/MapServer';
        //var agsType = new  gmaps.ags.MapType(url,{name:'NICMap'});
        //map.mapTypes.set('nicmap', agsType);
       // map.setMapTypeId('nicmap');
	...
	_add_chart_type: function() {
		...
		var chart_type= '<select onchange="DataViz._initialize_chart(null, false, this.value)" name="chart_type" id="chart_type" class="span11">'
	...
	_populate_filter: function() {
		...
		for(i in where) {
			...
			'<div class="row-fluid">'+
					...
					'<span class="label">'+where[i].statement_label+'</span> </div>'+
	...
	_populat_chart_data_series: function(columnKeys) {
		...
		DataViz._initialize_chart(null, false, chart_type);
	},
	...
	addSeries: function() {
		...
		'<div class="row-fluid">'+
				'<div class="span10">' +
				'<span class="label">' + DataViz._cache_data.getColumnLabel(parseInt(columnKey)) + '</span>'+
