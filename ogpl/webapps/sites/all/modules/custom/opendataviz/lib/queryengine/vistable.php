...
abstract class vistable {
    ...
    protected function make_order($elist, $order, &$cols, &$exprs)
    {
        foreach ($elist as &$col) {
            ...
            if (!isset($exprs[$s])) {
                $cols[] = array('id' =>  $col['id'], 'label' => $s, 'type' => $col['type']);
                $exprs[$s] = $col;
            }
            $order[$s] = $dir;
        }
    ...
    public function query_filter(&$rows, $query)
    {
        ...
        foreach ($query['select'] as &$col) {
            ...
            if($col['id'] == '' && $col['is_aggregate']){
            	$col['id'] = ++$_c_id;
            }
            
            $c = array('id' => $col['id'], 'label' => $s, 'type' => $col['type']);
            ...
            $cols[] = $c;
            $exprs[$s] =& $col;
            $_c_id = $col['id']!=''?$col['id']:$_c_id; //prev value
        }
        ...
        if ($table) {
            ...
            foreach ($cols as &$colref) {
                if (isset($colref['pattern'])) {
                    switch ($colref['type']) {
                    case 'number':
                        $colref['fmt'] = new VisNumberFormatter($this->locale,
                                                             VisNumberFormatter::PATTERN_DECIMAL,
                                                             $colref['pattern']);
                        if ($this->debug) {
                            print_r($colref['fmt']);
                        }
...
class csv_vistable extends vistable {
    ...
    /* 0 indexed value for column id meaning 0 == A */
    ...
    public function setup_table($data)
    {
        ...
        foreach ($row as &$id) {
            $type = 'string';
            if (preg_match('/^(.*) as (date|datetime|boolean|timeofday|number)$/',trim($id),$matches)) {
                $id = $matches[1];
                $type = $matches[2];
            }
            $id = str_replace(array("\r", "\r\n", "\n"), '', $id);
            $row_index = $this->getNameFromNumber($row_no);
            $this->fields[$row_index] = array(TYPE=>SIMPLE, VALUE=>$id, 'type' => $type, 'id' => $row_index);
            $row_no++;
        }
        $cols = $row;
        $this->table = array();
